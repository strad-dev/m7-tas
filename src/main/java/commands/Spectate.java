...
public class Spectate implements CommandExecutor {
	...
	// Per-spectator sync state. ConcurrentHashMap so netty thread can read it
	...
	/** True if spectating a fake player: their position mirrors the fake's and isn't their own presence
	 *  (e.g. Storm pad occupancy). */
	...
	private static void restorePlayerCollision(Player player) {
		// Not removed from no-collision team: real players stay in it all session (added on join). The team rule,
		// not setCollidable, is what stops player-vs-player pushing.
