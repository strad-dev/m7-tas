...
### Server/Boss Tick Ordering (INVARIANT)
- **Every server-side action MUST run at the start of the tick — before any player choreography.** "Server" = all boss/world simulation (the wither bosses, the Watcher, world setup), as opposed to the five player routines. This is what lets a player's mage-beam/melee hit on tick T read the server's already-finalized state (stun/enrage/HP/position, spawned mobs, opened portals) for that same tick, instead of the previous tick's. It removes the need for the +1-tick offsets that previously forced player hits to wait a tick for the server.
- Enforced by a single **server-priority heartbeat** in `BossScheduler` (`BossScheduler.start()`, called first in `M7tas.onEnable`). Because it is created at plugin-enable time, its Bukkit scheduler task id is the lowest of anything created at runtime, and CraftBukkit runs same-tick tasks in task-id order — so every registered ticker runs before per-run player choreography (which uses `Utils.scheduleTask` → raw `runTaskLater`, scheduled only when `/tas` runs, hence higher ids).
- Per-tick server SCANS register via `BossScheduler.addTicker(Runnable)` / `removeTicker(...)` and run at the start of the tick in registration order. Already migrated: Maxor's laser/stun scan, Storm's pad/crush poll (gated on `displayTick() % 20 == 0` so it polls only on phase ticks divisible by 20), and the **Watcher** (blood-room detection, portal detection, the travel/spawn animation, and all its one-shots). Necron is HP-threshold/event-driven (no per-tick scan). **Any new server action — boss OR Watcher OR world — that a player hit reads must register as a ticker / use `BossScheduler.schedule`, NOT its own `runTaskTimer`/`Utils.scheduleTask`.**
- **Entity MOVEMENT is a separate lane.** Boss aggro movers register via `BossScheduler.addMovementTicker(...)` / `removeMovementTicker(...)` and run in the **movement phase** — driven from the fake-player ticker AFTER fake `aiStep` (`FakePlayerManager`) — so a boss moves at the same point in the tick as the fakes ("where movement normally happens in vanilla"), NOT at the start of the tick. Consequence: the start-of-tick scans read the boss's PRE-move position, a deliberate one-tick lag matching vanilla entity ticking — so a player hit that depends on a position-triggered state change (e.g. Maxor's laser stun) legitimately keeps its +1-tick offset.
