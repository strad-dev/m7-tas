...
- **The wither bosses' per-tick scans run at the start of the tick — before any player choreography.** This lets a player's mage-beam/melee hit on tick T read the boss's already-finalized state (stun/enrage/HP) for that same tick. It removes the need for +1-tick offsets that previously forced player hits to wait a tick for the boss.
...
- Per-tick boss SCANS register via `BossScheduler.addTicker(Runnable)` / `removeTicker(...)` and run at the start of the tick in registration order. Migrated: Maxor's laser/stun scan and Storm's pad/crush poll — both gated on `displayTick() % 20 == 0` so they poll only on phase ticks divisible by 20 (the real-Hypixel 20-tick cycle). Necron is HP-threshold/event-driven (no per-tick scan). The **Watcher is NOT on the scheduler** — it uses ordinary `runTaskTimer`/`Utils.scheduleTask` (putting its chained spawn animations on the heartbeat introduced a cumulative per-mob spawn drift, so it was reverted). **Any new wither-boss mechanic that a player hit reads should register as a ticker / use `BossScheduler.schedule`, NOT its own `runTaskTimer`/`Utils.scheduleTask`.**
