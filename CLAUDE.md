...
- Per-tick server simulation registers via `BossScheduler.addTicker(Runnable)` / `removeTicker(...)` and runs in registration order (so a boss that needs "move, then scan" must register the mover first). Already migrated: the shared aggro mover (`WitherActions`), Maxor's laser/stun scan, Storm's pad/crush poll (an internal counter preserves its 20-tick cadence), and the **Watcher** (blood-room detection, portal detection, the travel/spawn animation, and all its one-shots). Necron is HP-threshold/event-driven (no per-tick scan). **Any new server action — boss OR Watcher OR world — that a player hit reads must register as a ticker / use `BossScheduler.schedule`, NOT its own `runTaskTimer`/`Utils.scheduleTask`.**
